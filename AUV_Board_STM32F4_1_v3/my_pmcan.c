...
// Mass Shifter Speed Control
	if(Flag.Mass_Run_CCW_Speed && GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_4) == 0 && !Flag.Pis_H && !Flag.Pis_T)
	{
		UPMCAN_Mass_SetOLoop_Duty(UPWM_DutyCycle_Mass,MOTOR_CCW);
		Flag.Mass_Run_CCW_Speed = false;
	}
	if(Flag.Mass_Run_CW_Speed && GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_5) == 0 && !Flag.Pis_H && !Flag.Pis_T)
	{
